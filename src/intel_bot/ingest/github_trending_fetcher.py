    ...
    *,
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            last_error = exc
            logger.warning('GitHub Trending fetch failed (attempt=%d): %s', attempt + 1, exc)

    raise RuntimeError(f'GitHub Trending fetch failed for {url_or_query!r}: {last_error}')
...
def parse_github_trending(html: str, source: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in soup.select('article.Box-row'):
        ...
        repo_path = normalize_title(link.get_text(' ', strip=True)).replace(' / ', '/').replace(' ', '')
        if not repo_path:
            continue

        raw_url = f'https://github.com/{repo_path}'
        description_node = row.select_one('p')
        description = normalize_title(description_node.get_text(' ', strip=True)) if description_node else ''
        language_node = row.select_one('[itemprop="programmingLanguage"]')
        language = normalize_title(language_node.get_text(' ', strip=True)) if language_node else ''
        stars_node = row.select_one('a[href$="/stargazers"]')
        stars = normalize_title(stars_node.get_text(' ', strip=True)) if stars_node else ''
        today_node = row.select_one('span.d-inline-block.float-sm-right')
        today = normalize_title(today_node.get_text(' ', strip=True)) if today_node else ''
        snippet = ' | '.join(part for part in (description, language, stars, today) if part)
        canonical = canonicalize_url(raw_url)
        content_hash = compute_content_hash(repo_path, canonical)
